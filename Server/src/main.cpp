...
int main() {
	...
	if (((client1 = accept(server, (SOCKADDR*)&clientAddr1, &clientAddrSize1)) != INVALID_SOCKET) && ((client2 = accept(server, (SOCKADDR*)&clientAddr2, &clientAddrSize2)) != INVALID_SOCKET)) {
		...
		DWORD tid1;
		DWORD tid2;
		HANDLE t1 = CreateThread(NULL, 0, serverReceive, &client1, 0, &tid1);
		if (t1 == NULL) {
			cout << "Ошибка создания потока" << WSAGetLastError() << endl;
		}
		HANDLE t2 = CreateThread(NULL, 0, serverReceive, &client2, 0, &tid2);
		if (t1 == NULL) {
			cout << "Ошибка создания потока" << WSAGetLastError() << endl;
		}
		HANDLE t3 = CreateThread(NULL, 0, serverSend, &client2, 0, &tid1);
		if (t2 == NULL) {
			cout << "Ошибка создания потока" << WSAGetLastError() << endl;
		}
		HANDLE t4 = CreateThread(NULL, 0, serverSend, &client1, 0, &tid2);
		if (t2 == NULL) {
			cout << "Ошибка создания потока" << WSAGetLastError() << endl;
		}
